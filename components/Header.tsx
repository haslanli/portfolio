    ...
    >
      <div className="container mx-auto px-6 flex justify-center">
        <motion.div 
          ...
          className={`
            ...
            ${isScrolled || isOpen 
                ? 'bg-white/90 dark:bg-navy-950/80 backdrop-blur-md border border-slate-200 dark:border-white/10 w-full md:w-auto shadow-xl shadow-black/5 dark:shadow-black/10' 
                : 'bg-transparent border border-transparent w-full md:w-auto'
            }
          `}
        >
            <Link to="/" className="flex items-center gap-2 text-slate-900 dark:text-white font-bold text-xl mr-8 group">
            ...
            <div className="flex items-center gap-2 ml-4">
                ...
                {/* Mobile Menu Button */}
                <button 
                  className="md:hidden text-slate-900 dark:text-white p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-full transition-colors"
      ...
      {isOpen && (
        <motion.div 
          ...
          className="fixed inset-0 z-40 bg-white/95 dark:bg-navy-950/95 backdrop-blur-md md:hidden flex items-center justify-center pt-20"
        >
            ...
            {navLinks.map((link) => (
              <Link 
                ...
                className={`text-2xl font-light ${
                  location.pathname === link.path ? 'text-indigo-600 dark:text-indigo-400 font-medium' : 'text-slate-800 dark:text-slate-300'
                }`}
              >
                {link.name}
              </Link>
